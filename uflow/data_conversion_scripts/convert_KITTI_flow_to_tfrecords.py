  ...
  for subdir in FLAGS.subdirs.split(','):
    ...
    for i in range(num_images):
      ...
      for image_file in image_files:
        ...
        # edit: 'r' to 'rb' otherwise data is interpreted as text
        image_data = tf.compat.v1.gfile.FastGFile(image_path, 'rb')
      ...
      if subdir == 'training':
        ...
        for version in ['noc', 'occ']:
          #edit FLAGS.subdir to subdir
          flow_path = os.path.join(data_dir, subdir, 'flow_' + version,
                                   image_files[0])
          # edit: 'r' to 'rb' otherwise data is interpreted as text
          flow_data = tf.compat.v1.gfile.FastGFile(flow_path, 'rb')
          flow_data = flow_data.read()
